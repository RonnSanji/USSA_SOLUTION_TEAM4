...
public class ManageProductWindow extends JPanel {
	...
	private String[] columns = new String[] { "Product Id", "Product Name", "Product Description", "Quantity Available",
			"Price", "Bar Code", "Reorder Quantity", "Order Quantity" };
	...
	public ManageProductWindow() {
		...
		btnAddNewProduct.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				AddProduct addProductWindow = new AddProduct(FileDataWrapper.productMap, manageProductWindow);
		...
		btnRemoveProduct.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					// StoreConstants.SELECT_PRODUCT, "Error",
				...
				if (selectedRow != -1) {
					...
					// JOptionPane.showConfirmDialog(table, "Confirm to remove
					// this Product?",
					...
					if (option == 0) {
						...
						// update the .dat file
						updateDatFile();
						DisplayUtil.displayAcknowledgeMessage(table, StoreConstants.PRODUCT_REMOVED_SUCCESSFULLY);
					}
		...
		btnEditProduct.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					DisplayUtil.displayValidationError(table, StoreConstants.SELECT_PRODUCT);
					// JOptionPane.showMessageDialog(table, "Please select at
					// least one row", "Error", JOptionPane.ERROR_MESSAGE);
				}
				if (selectedRow != -1) {
		...
		if (comboBoxProductSearch.getSelectedItem() != null) {
			...
			for (Product product : productList) {
				if (searchBy == comboBoxSearchByItem[0]) {
					if (product.getProductName().toUpperCase().contains(txtProductSearch.getText().toUpperCase())) {
						productListResult.add(product);
					}
				} else if (searchBy == comboBoxSearchByItem[1]) {
					if (product.getProductId().toUpperCase().contains(txtProductSearch.getText().toUpperCase())) {
						productListResult.add(product);
					}
				}
			}
